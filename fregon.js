...
if (fila_id == 1) {
		...
		if(event.which == 81){
			...
			$("#fila_id").parent().parent().css("background-color","#FFFFFF");
			$("b#fila_id").remove();
		}

		if (event.which == 13 || event.which == 107) {
			//alert($("#fila_id").parent().parent().html());
			var cantidad_i = parseFloat($("#fila_id").parent().parent().children("td:eq(2)").html())
			var precio     = parseFloat($("#fila_id").parent().parent().children("td:eq(3)").html())
			var cantidad   = cantidad_i + 1;
			var total      = cantidad * precio;

			$("#fila_id").parent().parent().children("td:eq(2)").html(cantidad);
			$("#fila_id").parent().parent().children("td:eq(4)").html(total);
